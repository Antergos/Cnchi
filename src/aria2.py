...
def tell_active(self, rpc_uid, keys):
    """ This method returns  a  list  of  active  downloads. """
    active = None
    try:
        s = xmlrpc.client.ServerProxy(ARIA2_URL)
        active = s.aria2.tellActive(rpc_uid, keys)
    except (xmlrpc.client.Fault, ConnectionRefusedError, BrokenPipeError) as err:
        logging.debug(_("Can't call Aria2. Error Output: %s"), err)
    finally:
        return active

def add_metalink(self, rpc_uid, metalink):
    ...
    if metalink != None:
        try:
            ...
            gids = s.aria2.addMetalink(rpc_uid, binary_metalink)
...
def add_uris(self, rpc_uid, uris):
    """ This method adds an uri list (of the same file) to the aria2's downloads list """
    gid = None
    try:
        s = xmlrpc.client.ServerProxy(ARIA2_URL)
        gid = s.aria2.addUri(rpc_uid, uris)
    except (xmlrpc.client.Fault, ConnectionRefusedError, BrokenPipeError, OverflowError) as err:
        logging.error("Can't add metalink to Aria2. Error Output: %s", err)
    finally:
        return gid

def purge_download_result(self, rpc_uid):
    """ This method purges completed/error/removed downloads to free memory. """
    try:
        s = xmlrpc.client.ServerProxy(ARIA2_URL)
        s.aria2.purgeDownloadResult(rpc_uid)
    except (xmlrpc.client.Fault, ConnectionRefusedError, BrokenPipeError) as err:
        logging.debug(_("Can't call Aria2. Error Output: %s"), err)

def shutdown(self, rpc_uid):
    """ This method shuts down aria2. """
    try:
        s = xmlrpc.client.ServerProxy(ARIA2_URL)
        s.aria2.shutdown(rpc_uid)
    except (xmlrpc.client.Fault, ConnectionRefusedError, BrokenPipeError) as err:
        logging.debug(_("Can't call Aria2. Error Output: %s"), err)

def run_as_daemon(self, pacman_cache_dir, max_concurrent_downloads):
    ...
    aria2_options = [
        ...
        "--dir=%s" % pacman_cache_dir,  # The directory to store the downloaded file(s).
