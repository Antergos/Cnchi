...
class InstallationProcess(multiprocessing.Process):
    ...
    def install_packages_group(package_type, download_only=False)      
        pacman_options = {}
        if download_only:
    ...
    def install_packages(self):
        ...
        First we try to download all packages, then we retry all those that failed (if any)
        Then we install all downloaded (but if some didn't, alpm retries again, so we retry three times!)
        After installing all packages, we retry all those that failed (if any)
        And that's it!
        Note: We call 'group' to our organization of packages, that has nothing to do with Arch groups.
        """
        ...
        global_percent = 0
        for package_type in self.packages:
            result = self.install_packages_group(package_type, download_only=True)
            if result == 1:
                if package_type == "common":
                    raise InstallError("Can't download group 'common'. This group is essential. Cnchi can't continue.")
                else:
                    txt = _("Couldn't download group '%s'. Cnchi will continue and try again this download later.") % package_type
                    logging.error(txt)
                    download_falied.append(package_type)
        ...
        if len(download_failed) > 0:
            logging.warning("Couldn't download %s group(s), will retry now...", ','.join(download_failed))
            # Retry, just once, all failed downloads...
            global_percent = 0
            for failed in download_failed:
                result = self.install_packages_group(failed, download_only=True)
                global_percent += step_global * self.number_of_packages_by_type(alpm, package_type)
                self.queue_event('global_percent', global_percent)
                self.queue_event('local_percent', 0)
            self.queue_event('global_percent', 0)
        ...
        global_percent = 0
        for package_type in self.packages:
            result = self.install_packages_group(package_type)
            if result == 1:
                if package_type == "common":
                    raise InstallError("Can't install necessary packages. Cnchi can't continue.")
                else:
                    txt = _("Couldn't install group '%s'. Cnchi will continue but this group of packages won't be installed.") % package_type
                    logging.error(txt)
                    install_failed.append(package_type)
        ...
        if len(install_failed) > 0:
            logging.warning("Couldn't install %s group(s), will retry now...", ','.join(install_failed))
            # Retry, just once, all failed installs...
            global_percent = 0
            for failed in install_failed:
                result = self.install_packages_group(failed)
                global_percent += step_global * self.number_of_packages_by_type(alpm, package_type)
                self.queue_event('global_percent', global_percent)
                self.queue_event('local_percent', 0)
